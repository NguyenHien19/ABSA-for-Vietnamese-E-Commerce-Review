...
class SentenceDataset(Dataset):
    ...
    def __init__(self, fname, tokenizer, opt, vocab_help):
        ...
        for obj in tqdm(parse(fname), total=len(parse(fname)), desc="Training examples"):
            ...
            #print(len(obj['short']))
            #print(len(obj['short'][0]))

            #print("len of mask 0: ", len(mask_0))
            #print("len of mask 1: ", len(mask_1))

            #print("len of mask 2: ", len(mask_2))
            ...
            for i in range(0, len(obj['short'])):
                for j in range(0, len(obj['short'])):
                    mask_0[i][j] = 0
                    if obj['short'][i][j] == 1:
                        mask_1[i][j] = 0 
                        mask_2[i][j] = 0
...
def build_embedding_matrix(vocab, embed_dim, data_file):
    if os.path.exists(data_file):
        ...
        fname = './PhoW2V/word2vec_vi_words_300dims.txt'
